...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            Emp result = Emps.Where(emp => emp.Job == "Frontend programmer").OrderByDescending(emp => emp.HireDate)
                .FirstOrDefault();
            return result;
        }
        ...
        {
            int result = arr.GroupBy(x => x).Where(g => g.Count() % 2 != 0).Select(g => g.Key).First();
            //result=
            return result;
        }
        ...
        {
            IEnumerable<Dept> result = Depts.Where(dept => Emps.Count(emp => emp.Deptno == dept.Deptno) == 5 || Emps.Count(emp => emp.Deptno == dept.Deptno) == 0)
        ...
        {
            IEnumerable<Dept> result = Emps.Where(emp => emp.Job.Contains("A"))
                .GroupBy(emp => emp.Job)
                .Where(group => group.Count() > 2)
                .OrderByDescending(group => group.Count())
                .Select(group => new Dept {Dname = group.Key});
            //result =
            return result;
        }
        ...
        {
            IEnumerable<Dept> result = Emps.SelectMany(emp => Depts, (emp, dept) => dept);
            //result =
            return result;
        }
    }
